...
""" 
Uncomment additional paks you want. Paks like textures/other localization paks are commented out since they're fairly huge and don't need to be extracted unless you really need them.
"""
...
if default_data_path is not None:
    ...
    and not "DefEd/Data" in str(default_data_path.absolute())):
...
default_extract_path = Path( __file__ ).parent.joinpath(f"GameData_Extracted_{int(datetime.datetime.now().timestamp())}")
...
def extract_pak(f:Path, divine:Path, output:Path):
    targs = [
        str(divine.absolute()),
        "-g dos2de",
        f'-s "{f.absolute()}"',
    ...
    print(f'[divine] Exited with code ({proc.returncode}).')
...
def main():
    ...
    if args.data is not None and args.output is not None and args.divine is not None:
        ...
        for pak_name in data_paks:
            ...
            if pak_path.exists():
                print(f"Extracting package '{pak_name}'.")
                if extract_pak(pak_path, divine_path, output_dir):
                    successes = successes + 1
                else:
                    errors = errors + 1
            else:
                print(f"Package '{pak_path}' does not exist.")
